...
import fun.rich.features.impl.combat.NeuroAuraExec;
...
public class Aura extends Module {
    ...
    private final NeuroAuraExec neuroExecEngine = new NeuroAuraExec();
    ...
    // --- Neuro / SpookyTime helpers ---

    public boolean neuroEnabled() {
        return aimMode.isSelected("SpookyTime");
    }

    // In this Aura мы всегда выполняем Neuro, отдельного "обучающего" режима нет
    public boolean neuroExec() {
        return neuroEnabled();
    }

    public boolean neuroShakeEnabled() {
        // Включаем shake при потере цели / выключении для SpookyTime
        return neuroEnabled();
    }

    public float neuroSmoothValue() {
        // Базовое значение плавности как в NeuroAura
        float v = 0.76f;
        if (v < 0.20f) v = 0.20f;
        if (v > 0.95f) v = 0.95f;
        return v;
    ...
    public NeuroAuraExec getNeuroExecEngine() {
        return neuroExecEngine;
    }
    ...
    public void tick(TickEvent e) {
        // Обновляем Neuro-исполнитель для SpookyTime
    ...
    public void onRotationUpdate(RotationUpdateEvent e) {
        ...
        switch (e.getType()) {
            case EventType.PRE -> {
                // Neuro может занять управление поворотом сам
                if (neuroEnabled() && neuroExecEngine.handlePreRotation(this)) {
                    target = null;
                    lastTarget = null;
                    return;
                }
                ...
                target = updateTarget();
                if (target != null) {
                    if (neuroEnabled() && neuroExec()) {
                        neuroExecEngine.onTargetSelected(this, target);
                    }
                    rotateToTarget(getConfig());
                    lastTarget = target;
                } else if (neuroEnabled()) {
                    // Попытаться красиво вернуть взгляд при потере цели
                    if (neuroExecEngine.handleNoTarget(this, lastTarget)) {
                        lastTarget = null;
                        return;
                    }
                    lastTarget = null;
    ...
    private void rotateToTarget(StrikerConstructor.AttackPerpetratorConfigurable config) {
        ...
        // Для режима SpookyTime сначала пробуем Neuro-исполнитель
        if (neuroEnabled() && neuroExecEngine.tryApplyExecRotation(this, config, attackHandler, controller, rotationConfig)) {
            return;
        }
    ...
    public StrikerConstructor.AttackPerpetratorConfigurable getConfig() {
        ...
        // Позволяем Neuro скорректировать точку наведения для SpookyTime
        if (neuroEnabled()) {
