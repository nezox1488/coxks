...
public class StrikeManager implements QuickImports {
    ...
    private boolean isPlayerInCriticalState(PlayerSimulation simulated, int ticks) {
        boolean fall = simulated.fallDistance > (Aura.getInstance().getAimMode().isSelected("SpookyTime") ? Calculate.getRandom(0, 0.2) : 0) && (simulated.fallDistance < 0.08 || !PlayerSimulation.simulateLocalPlayer(ticks + 1).onGround);;
