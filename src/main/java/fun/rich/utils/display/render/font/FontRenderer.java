...
import fun.rich.utils.display.render.font.entry.DrawEntry;
import fun.rich.utils.display.render.font.glyph.Glyph;
import fun.rich.utils.display.render.font.glyph.GlyphMap;
...
public class FontRenderer implements QuickImports {
    ...
    public void drawRainbowString(MatrixStack matrix, java.lang.String text, double x, double y, int color1, int color2) {
        TextFactoryEvent event = new TextFactoryEvent(text);
        EventManager.callEvent(event);
        text = event.getText();
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (glyph != null) {
                if (glyph.value() != ' ') {
                    float t = (float) (Math.sin((time + i * 100) / 500.0) + 1.0) / 2.0f;
    ...
    public void drawHalfSplitString(MatrixStack matrix, java.lang.String text, double x, double y, int color1, int color2) {
        TextFactoryEvent event = new TextFactoryEvent(text);
        EventManager.callEvent(event);
        text = event.getText();
        ...
        if (visibleCharCount == 1) {
            for (int i = 0; i < chars.length; i++) {
                ...
                if (glyph != null && glyph.value() != ' ') {
                    Identifier i1 = glyph.owner().bindToTexture;
                    DrawEntry entry = new DrawEntry(xOffset, yOffset, color1, glyph);
                    GLYPH_PAGE_CACHE.computeIfAbsent(i1, integer -> new ObjectArrayList<>()).add(entry);
                    xOffset += glyph.width();
                }
            }
            if (!GLYPH_PAGE_CACHE.isEmpty()) drawHalfSplitGlyphs(matrix, x, y, totalWidth, color1, color2);
            GLYPH_PAGE_CACHE.clear();
            return;
        }
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (glyph != null) {
                if (glyph.value() != ' ') {
                    float midPoint = totalWidth / 2f;
                    float glyphStart = xOffset;
                    float glyphEnd = xOffset + glyph.width();
                    int color;
                    if (glyphStart < midPoint && glyphEnd > midPoint) {
                        float partBeforeMid = midPoint - glyphStart;
                        float partAfterMid = glyphEnd - midPoint;
                        color = (partBeforeMid >= partAfterMid) ? color1 : color2;
                        
                    } else {
                        color = (glyphEnd <= midPoint) ? color1 : color2;
        ...
        if (!GLYPH_PAGE_CACHE.isEmpty()) {
            drawHalfSplitGlyphs(matrix, x, y, totalWidth, color1, color2);
        }
        GLYPH_PAGE_CACHE.clear();
    }

    private void drawHalfSplitGlyphs(MatrixStack matrix, double x, double y, float totalWidth, int color1, int color2) {
        ...
        for (Identifier identifier : GLYPH_PAGE_CACHE.keySet()) {
            ...
            BufferBuilder buffer = Tessellator.getInstance().begin(TRIANGLES, POSITION_TEXTURE_COLOR);
            for (DrawEntry drawEntry : GLYPH_PAGE_CACHE.get(identifier)) {
                ...
                if (topAlpha == 0) topAlpha = 255;
                int topRed = (topColor >> 16) & 0xFF;
                int topGreen = (topColor >> 8) & 0xFF;
                int topBlue = topColor & 0xFF;
                topColor = (topAlpha << 24) | (topRed << 16) | (topGreen << 8) | topBlue;

                int bottomColor = color2;
                int bottomAlpha = (bottomColor >> 24) & 0xFF;
                if (bottomAlpha == 0) bottomAlpha = 255;
                int bottomRed = (bottomColor >> 16) & 0xFF;
                int bottomGreen = (bottomColor >> 8) & 0xFF;
                int bottomBlue = bottomColor & 0xFF;
                bottomColor = (bottomAlpha << 24) | (bottomRed << 16) | (bottomGreen << 8) | bottomBlue;

                buffer.vertex(matrix4f, x1, y1, 0).texture(u1, v1).color(topColor);
                buffer.vertex(matrix4f, x1 + width, y1, 0).texture(u2, v1).color(topColor);
                buffer.vertex(matrix4f, x1, y1 + height, 0).texture(u1, v2).color(topColor);

                buffer.vertex(matrix4f, x1, y1 + height, 0).texture(u1, v2).color(bottomColor);
                buffer.vertex(matrix4f, x1 + width, y1, 0).texture(u2, v1).color(bottomColor);
                buffer.vertex(matrix4f, x1 + width, y1 + height, 0).texture(u2, v2).color(bottomColor);
            }
