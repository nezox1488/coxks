...
public record BuiltText(
    ...
    public void render(Matrix4f matrix, float x, float y, float z) {
		...
		if (this.rainbow) {
			this.font.applyRainbowGlyphs(matrix, builder, this.text, this.size,
				(this.thickness + this.outlineThickness * 0.5f) * 0.5f * this.size, this.spacing,
					x, y + this.font.getMetrics().baselineHeight() * this.size, z, this.rainbowColor1, this.rainbowColor2);
		} else {
